...
#   - Performs preflight checks for DB/Redis and optional Celery worker ping.
...
#   - RUN20_PREFLIGHT_STRICT: 1=fail on DB/Redis unreachable (default), 0=warn.
#   - RUN20_CHECK_WORKER: 1=check Celery worker ping (default), 0=skip.
#   - RUN20_REQUIRE_WORKER: 1=fail when no worker responds, 0=warn (default).
#
# Notes:
...
RUN20_PREFLIGHT_STRICT="${RUN20_PREFLIGHT_STRICT:-1}"
RUN20_CHECK_WORKER="${RUN20_CHECK_WORKER:-1}"
RUN20_REQUIRE_WORKER="${RUN20_REQUIRE_WORKER:-0}"
...
# Also normalize REDIS_URL from redis://redis:6379/... to localhost.
if [ -f "$ROOT/.env" ]; then
    ...
    REDIS_URL_FROM_ENV_FILE="$(grep -E '^REDIS_URL=' "$ROOT/.env" | tail -n 1 | cut -d'=' -f2-)"
    if [ -n "$REDIS_URL_FROM_ENV_FILE" ]; then
        export REDIS_URL="$REDIS_URL_FROM_ENV_FILE"
    fi
fi
...
extract_url_host() {
    local url="$1"
    printf '%s' "$url" | sed -E 's#^[a-zA-Z][a-zA-Z0-9+.-]*://([^/@]+@)?([^:/?#]+).*$#\2#'
}
...
extract_url_port() {
    ...
    local parsed
    parsed="$(printf '%s' "$url" | sed -nE 's#^[a-zA-Z][a-zA-Z0-9+.-]*://([^/@]+@)?[^:/?#]+:([0-9]+).*$#\2#p')"
    if [ -n "$parsed" ]; then
        printf '%s' "$parsed"
    else
...
if [ -n "${DATABASE_URL:-}" ]; then
    ...
    DB_PORT="$(extract_url_port "$DATABASE_URL" "5432")"
fi
...
if check_tcp_with_python "$DB_HOST" "$DB_PORT"; then
    echo "[OK] PostgreSQL reachable at ${DB_HOST}:${DB_PORT}"
else
    MSG="[Error] PostgreSQL unreachable at ${DB_HOST}:${DB_PORT}. Start DB and retry."
    if [ "$RUN20_PREFLIGHT_STRICT" = "1" ]; then
        echo "$MSG" >&2
        exit 1
    fi
    echo "${MSG/[Error]/[Warn]}" >&2
fi

if check_tcp_with_python "$REDIS_HOST" "$REDIS_PORT"; then
    echo "[OK] Redis reachable at ${REDIS_HOST}:${REDIS_PORT}"
else
    MSG="[Error] Redis unreachable at ${REDIS_HOST}:${REDIS_PORT}. Start Redis and retry."
    if [ "$RUN20_PREFLIGHT_STRICT" = "1" ]; then
        echo "$MSG" >&2
        exit 1
    fi
...
if [ "$RUN20_CHECK_WORKER" = "1" ]; then
    WORKER_COUNT="$( (cd "$BACKEND_DIR" && "$VENV_PY" - <<'PY'
import os
from worker import celery_app

inspector = celery_app.control.inspect(timeout=1)
ping = inspector.ping() if inspector else None
count = len(ping) if ping else 0
print(count)
PY
) 2>/dev/null || echo "0")"
    ...
    if [ "$WORKER_COUNT" -gt 0 ] 2>/dev/null; then
        ...
        MSG="[Warn] No active Celery worker responded. Jobs may stay queued."
        if [ "$RUN20_REQUIRE_WORKER" = "1" ]; then
            echo "${MSG/[Warn]/[Error]}" >&2
            exit 1
        fi
        echo "$MSG" >&2
    fi
