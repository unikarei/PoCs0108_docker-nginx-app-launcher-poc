...
rem
rem Purpose:
rem   Stage all current changes, create one commit, and push that commit to the
rem   current remote branch without creating or pushing a tag.
rem
rem How it works:
rem   1. Changes to the working tree are staged with git add -A.
rem   2. The current branch is detected for display and for push target.
rem   3. A commit message is requested interactively from the user.
rem   4. If the push is rejected because the remote has diverged, the script
rem      offers an optional force-push path after explicit confirmation.
rem
rem Notes:
rem   - This script is intentionally interactive.
rem   - It assumes you want all local changes staged, not only modified files.
rem   - Tagging is handled separately by run04_tag.bat / run04_tag.sh.
rem ========================================================================
