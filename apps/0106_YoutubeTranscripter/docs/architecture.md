...
- Editor描画時は HTML エスケープ後に記法を変換し、XSS を防止する。

7. 結果本文編集（UI/API）
- Results の各本文は共通 `contenteditable` Editor で編集する。
- 保存形式は既存の `**...**` / `==...==` 記法とし、貼り付けHTMLは保存しない。
- APIは許可された本文種別と対象IDだけを更新する。
- エクスポートとLLM入力では装飾記法を除去し、意味本文だけを渡す。
